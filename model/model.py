...
class Model(object):
    def single_model(self, gpu_id):
        ...
        if Hp.num_speakers >1:
            ...
            memory = tf.concat([text, prosody], axis = -1, name = "memory")  # [batch, Tx, Dt+Ds ]
    ...
    def __init__(self, mode, inputs = None):
        with tf.device('/cpu:0'):
            ...
            if inputs is not None:
                ...
                self.max_len_per_batch = tf.constant(500)
                    ...
                    with tf.name_scope('tower_%d'%gpu_id):
                        with tf.variable_scope('cpu_variables', reuse = gpu_id>0):
                            if mode == "synthes":
                                ...
                                self.inputs_speaker[gpu_id] = tf.placeholder(tf.int32, shape=[None,1], name = "inputs_speaker") #speaker id [batch, 1]
